...
str_ori = {'Si', 'Sg', 'LAC', 'Pg', 'Y4', 'BY', 'TY', 'Aa624', 'PA14'};
...
%{'PA14'}    {'Aa624'}    {'Y4'}    {'BY'}    {'TY'}    {'Pg'}
%{'PA14'}    {'Aa624'}    {'Y4'}    {'BY'}    {'TY'}    {'Pg'}    {'Sg'}    {'LAC'}

id_species=1;% id = 1; 6
% Aa species 
% id = 1; 6
if (id_species==1)  | (id_species==6)
load('Species=4_table_Num.mat');
...
for idx = 1:2
    ...
    if idx==1, scatter(XX_Si, YY_Si,20,'k');ylim([-Inf 20]);
                    idx_str = 'before';
        xlabel('log_2(mRNA)'),ylabel('log_2(observed protein)');
        legend({['N=',num2str(length(YY_c))],['\rho_{before}=',num2str(round(100*array_before)/100)]} ,'Location','northwest','Box','off') ;
    end
    if idx==2, scatter(YY_c,YY_Si,20,'r');ylim([-Inf 20]);xlim([4 20]);
                    idx_str = 'after';
        ylabel('log_2(observed protein)'),xlabel('log_2(predicted protein)');
        legend({['N=',num2str(length(YY_c))],['\rho_{after}=',num2str(round(100*array_after)/100)]} ,'Location','northwest','Box','off') ;
    end
    saveas(gcf,['Scatter_GramN_usingAa= ',Str_gramN{id_species},idx_str,'_PLOT.pdf'])  
end
%%
